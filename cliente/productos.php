...
if (!empty($_POST)) {
    ...
    if (empty($_POST['nombre']) || empty($_POST['descripcion']) || empty($_POST['cantidad']) || empty($_POST['precio']) || empty($_POST['color']) || empty($_POST['tamanio']) || empty($_FILES['foto']['name']) || empty($_POST['idcat'])) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['foto'])) {
            ...
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $rutaArchivo)) {
                ...
                $consulta = mysqli_query($conexion, "INSERT INTO productos (nombre, descripcion, cantidad, precio, color, tamanio, foto, idcat) VALUES ('$nombre', '$descripcion', '$cantidad', '$precio', '$color', '$tamanio', '$foto', '$idcate')");
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        if ($_SESSION['rol'] == 1) {;     ?>
                            <!--BOTON DE EDITAR-->
                            <td><a href="../servidor/editar_productos.php? id= <?php echo $datos['idprod'] ?>">
